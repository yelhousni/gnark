...
import (
	...
	plonk_bw6672 "github.com/consensys/gnark/internal/backend/bw6-672/plonk"
	...
	fr_bw6672 "github.com/consensys/gnark-crypto/ecc/bw6-672/fr"
...
func NewSRS(ccs frontend.CompiledConstraintSystem) (kzg.SRS, error) {
	...
	case *cs_bw6672.SparseR1CS:
		alpha, err := rand.Int(rand.Reader, fr_bw6672.Modulus())
		if err != nil {
			return nil, err
		}
		return kzg_bw6672.NewSRS(kzgSize, alpha)
...
func Setup(ccs frontend.CompiledConstraintSystem, kzgSRS kzg.SRS) (ProvingKey, VerifyingKey, error) {
	...
	case *cs_bw6672.SparseR1CS:
		return plonk_bw6672.Setup(tccs, kzgSRS.(*kzg_bw6672.SRS))
	default:
		panic("unrecognized R1CS curve type")
	}
}
...
func Prove(ccs frontend.CompiledConstraintSystem, pk ProvingKey, fullWitness frontend.Circuit) (Proof, error) {
	...
	case *cs_bw6672.SparseR1CS:
		...
		return plonk_bw6672.Prove(tccs, pk.(*plonk_bw6672.ProvingKey), w)
...
func Verify(proof Proof, vk VerifyingKey, publicWitness frontend.Circuit) error {
	...
	case *plonk_bw6672.Proof:
		w := witness_bw6672.Witness{}
		if err := w.FromPublicAssignment(publicWitness); err != nil {
			return err
		}
		return plonk_bw6672.Verify(_proof, vk.(*plonk_bw6672.VerifyingKey), w)
...
func ReadAndProve(ccs frontend.CompiledConstraintSystem, pk ProvingKey, witness io.Reader) (Proof, error) {
	...
	case *cs_bls12377.SparseR1CS:
		...
		return proof, nil
	case *cs_bw6672.SparseR1CS:
		_pk := pk.(*plonk_bw6672.ProvingKey)
		w := witness_bw6672.Witness{}
		if _, err := w.LimitReadFrom(witness, expectedSize); err != nil {
			return nil, err
		}
		proof, err := plonk_bw6672.Prove(tccs, _pk, w)
		if err != nil {
			return proof, err
		}
		return proof, nil
...
func ReadAndVerify(proof Proof, vk VerifyingKey, witness io.Reader) error {
	...
	case *plonk_bw6672.Proof:
		_vk := vk.(*plonk_bw6672.VerifyingKey)
		w := witness_bw6672.Witness{}
		if _, err := w.LimitReadFrom(witness, expectedSize); err != nil {
			return err
		}
		return plonk_bw6672.Verify(_proof, _vk, w)
