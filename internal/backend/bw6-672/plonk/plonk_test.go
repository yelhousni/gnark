...
import (
	...
	bw6_672plonk "github.com/consensys/gnark/internal/backend/bw6-672/plonk"

	"bytes"
	"github.com/consensys/gnark-crypto/ecc/bw6-672/fr/kzg"
	"math/big"
	"testing"
...
func BenchmarkSetup(b *testing.B) {
	...
	b.Run("setup", func(b *testing.B) {
		for i := 0; i < b.N; i++ {
			_, _, _ = bw6_672plonk.Setup(ccs.(*cs.SparseR1CS), srs)
		}
	})
}
...
func BenchmarkProver(b *testing.B) {
	...
	pk, _, err := bw6_672plonk.Setup(ccs.(*cs.SparseR1CS), srs)
	if err != nil {
		b.Fatal(err)
	}
	...
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		_, err = bw6_672plonk.Prove(ccs.(*cs.SparseR1CS), pk, fullWitness)
		if err != nil {
			b.Fatal(err)
		}
	}
...
func BenchmarkVerifier(b *testing.B) {
	...
	pk, vk, err := bw6_672plonk.Setup(ccs.(*cs.SparseR1CS), srs)
	if err != nil {
		b.Fatal(err)
	}

	proof, err := bw6_672plonk.Prove(ccs.(*cs.SparseR1CS), pk, fullWitness)
	if err != nil {
		panic(err)
	}
	...
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		_ = bw6_672plonk.Verify(proof, vk, publicWitness)
	}
}
...
func BenchmarkSerialization(b *testing.B) {
	...
	// ---------------------------------------------------------------------------------------------
	// bw6_672plonk.ProvingKey binary serialization
	b.Run("pk: binary serialization (bw6_672plonk.ProvingKey)", func(b *testing.B) {
		b.ResetTimer()
		for i := 0; i < b.N; i++ {
	...
	b.Run("pk: binary deserialization (bw6_672plonk.ProvingKey)", func(b *testing.B) {
		var buf bytes.Buffer
		_, _ = pk.WriteTo(&buf)
		var pkReconstructed bw6_672plonk.ProvingKey
		b.ResetTimer()
		for i := 0; i < b.N; i++ {
	...
	// ---------------------------------------------------------------------------------------------
	// bw6_672plonk.Proof binary serialization
	b.Run("proof: binary serialization (bw6_672plonk.Proof)", func(b *testing.B) {
		b.ResetTimer()
		for i := 0; i < b.N; i++ {
			var buf bytes.Buffer
	...
	b.Run("proof: binary deserialization (bw6_672plonk.Proof)", func(b *testing.B) {
		var buf bytes.Buffer
		_, _ = proof.WriteTo(&buf)
		var proofReconstructed bw6_672plonk.Proof
		b.ResetTimer()
		for i := 0; i < b.N; i++ {
			buf := bytes.NewBuffer(buf.Bytes())
