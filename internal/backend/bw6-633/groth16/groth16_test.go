...
func BenchmarkProver(b *testing.B) {
	...
	b.Run("prover", func(b *testing.B) {
		for i := 0; i < b.N; i++ {
			_, _ = bw6_633groth16.Prove(r1cs.(*cs.R1CS), &pk, fullWitness, false)
		}
	})
}
...
func BenchmarkVerifier(b *testing.B) {
	...
	var vk bw6_633groth16.VerifyingKey
	bw6_633groth16.Setup(r1cs.(*cs.R1CS), &pk, &vk)
	proof, err := bw6_633groth16.Prove(r1cs.(*cs.R1CS), &pk, fullWitness, false)
	if err != nil {
		panic(err)
	}
...
func BenchmarkSerialization(b *testing.B) {
	...
	// ---------------------------------------------------------------------------------------------
	// bw6_633groth16.ProvingKey binary serialization
	b.Run("pk: binary serialization (bw6_633groth16.ProvingKey)", func(b *testing.B) {
		b.ResetTimer()
		for i := 0; i < b.N; i++ {
	...
	b.Run("pk: binary deserialization (bw6_633groth16.ProvingKey)", func(b *testing.B) {
		var buf bytes.Buffer
		_, _ = pk.WriteTo(&buf)
		var pkReconstructed bw6_633groth16.ProvingKey
		b.ResetTimer()
		for i := 0; i < b.N; i++ {
	...
	// ---------------------------------------------------------------------------------------------
	// bw6_633groth16.ProvingKey binary serialization (uncompressed)
	b.Run("pk: binary raw serialization (bw6_633groth16.ProvingKey)", func(b *testing.B) {
		b.ResetTimer()
		for i := 0; i < b.N; i++ {
			var buf bytes.Buffer
	...
	b.Run("pk: binary raw deserialization (bw6_633groth16.ProvingKey)", func(b *testing.B) {
		var buf bytes.Buffer
		_, _ = pk.WriteRawTo(&buf)
		var pkReconstructed bw6_633groth16.ProvingKey
		b.ResetTimer()
	...
	// ---------------------------------------------------------------------------------------------
	// bw6_633groth16.ProvingKey binary serialization (cbor)
	b.Run("pk: binary cbor serialization (bw6_633groth16.ProvingKey)", func(b *testing.B) {
		b.ResetTimer()
		for i := 0; i < b.N; i++ {
			var buf bytes.Buffer
	...
	b.Run("pk: binary cbor deserialization (bw6_633groth16.ProvingKey)", func(b *testing.B) {
		var buf bytes.Buffer
		enc := cbor.NewEncoder(&buf)
		enc.Encode(&pk)
		var pkReconstructed bw6_633groth16.ProvingKey
	...
	// ---------------------------------------------------------------------------------------------
	// bw6_633groth16.Proof binary serialization (cbor)
	b.Run("proof: binary cbor serialization (bw6_633groth16.Proof)", func(b *testing.B) {
		b.ResetTimer()
		for i := 0; i < b.N; i++ {
			var buf bytes.Buffer
			enc := cbor.NewEncoder(&buf)
	...
	b.Run("proof: binary cbor deserialization (bw6_633groth16.Proof)", func(b *testing.B) {
		var buf bytes.Buffer
		enc := cbor.NewEncoder(&buf)
		enc.Encode(&proof)
		var proofReconstructed bw6_633groth16.Proof
