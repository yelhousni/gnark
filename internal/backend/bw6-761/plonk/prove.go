...
func Prove(spr *cs.SparseR1CS, pk *ProvingKey, fullWitness bw6_761witness.Witness) (*Proof, error) {
...
func evalConstraints(pk *ProvingKey, evalL, evalR, evalO, qk []fr.Element) []fr.Element {
	...
	utils.Parallelize(len(evalQr), func(start, end int) {
		var buf fr.Element
		for i := start; i < end; i++ {
...
func evalIDCosets(pk *ProvingKey) (id, uid, uuid polynomial.Polynomial) {
	...
	id[0] = pk.DomainNum.FinerGenerator
	zn := fr.One()
	...
	id1 := id[1*pk.DomainNum.Cardinality : 2*pk.DomainNum.Cardinality]
	id2 := id[2*pk.DomainNum.Cardinality : 3*pk.DomainNum.Cardinality]
	id3 := id[3*pk.DomainNum.Cardinality : 4*pk.DomainNum.Cardinality]

	copy(id1[1:], id[1:pk.DomainNum.Cardinality])
	copy(id2[1:], id[1:pk.DomainNum.Cardinality])
	copy(id3[1:], id[1:pk.DomainNum.Cardinality])
	id1[0] = u[1]
	id2[0] = u[2]
	id3[0] = u[3]
...
func evaluateCosets(poly, res []fr.Element, domain *fft.Domain) {
	...
	e0 := res[0*domain.Cardinality : 1*domain.Cardinality] //make([]fr.Element, domain.Cardinality)
	e1 := res[1*domain.Cardinality : 2*domain.Cardinality] //make([]fr.Element, domain.Cardinality)
	e2 := res[2*domain.Cardinality : 3*domain.Cardinality] //make([]fr.Element, domain.Cardinality)
	e3 := res[3*domain.Cardinality : 4*domain.Cardinality] //make([]fr.Element, domain.Cardinality)
...
func computeH(pk *ProvingKey, constraintsInd, constraintOrdering, startsAtOne polynomial.Polynomial, alpha fr.Element) (polynomial.Polynomial, polynomial.Polynomial, polynomial.Polynomial) {
	...
	utils.Parallelize(len(h), func(start, end int) {
		for i := start; i < end; i++ {
			...
			// TODO @gbotrel
			if i < int(pk.DomainNum.Cardinality) {
				h[i].Mul(&h[i], &uinv[0])
			} else if i < 2*int(pk.DomainNum.Cardinality) {
				h[i].Mul(&h[i], &uinv[1])
			} else if i < 3*int(pk.DomainNum.Cardinality) {
				h[i].Mul(&h[i], &uinv[2])
			} else {
				h[i].Mul(&h[i], &uinv[3])
			}
	...
	hcopy := make(polynomial.Polynomial, pk.DomainH.Cardinality)
	copy(hcopy, h)
	for i := 0; i < int(pk.DomainNum.Cardinality); i++ {
		h[4*i] = hcopy[i]
		h[4*i+1] = hcopy[i+1*int(pk.DomainNum.Cardinality)]
		h[4*i+2] = hcopy[i+2*int(pk.DomainNum.Cardinality)]
		h[4*i+3] = hcopy[i+3*int(pk.DomainNum.Cardinality)]
	}
